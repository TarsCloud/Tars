...
namespace JavaTarsPrxGenerator
{
    ...
    static void TarsWriteServiceDocComment(Printer *printer, const ServiceDescriptor *service)
    {
        printer->Print("/**\n");
        printer->Print(" */\n");
    }
                         ...
                         bool enable_deprecated)
    {
        ...
        Printer printer(out, '$');
        string package_name = ServiceJavaPackage(service->file(), flavor == ProtoFlavor::NANO);

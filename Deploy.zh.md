...
源码部署方式是了解Tars非常好的途径, 源码部署细节方式请参见 [here](https://github.com/TarsCloud/Tars/blob/master/Install.zh.md) 中的源码部署部分, 前列建议阅读.
...
- 自动制作(不包含运行环境), 具体参见 [here](https://github.com/TarsCloud/TarsDocker/blob/master/README.md) 
- 自动制作(包含运行环境), 具体参见 [here](https://github.com/TarsCloud/TarsDocker/blob/master/README.md) 
...
- 将tarsnode节点也容器化, 使用docker: tarscloud/tarsnode
- tars框架和tarsnode节点都作为pod部署在k8s上, pod运行的容器当成一台虚拟机
- 通过tars web发布服务到这些容器中运行

具体参见 [here](https://github.com/TarsCloud/TarsDocker/blob/master/README.md) 

**注意:docker内部提供了各种语言的运行环境, 简单的说可以把业务服务发布到这个docker内部运行(相当于把docker当成虚拟机)**
